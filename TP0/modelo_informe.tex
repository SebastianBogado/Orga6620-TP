...
\texttt{void sort(unsigned n,\\
		char* files[],\\
		line** (*sort\_func)(line** , unsigned ))}\\


Mañana sigo, Ale. Fijate si podés hacer que queden identados los parámetros (que todos empiecen a la misma altura). La idea es explicar cada parámetro, y después contar que acá se arma el vector con las líneas de todos los archivos y, si n es cero, se toma de stdin con la misma llamada: parseLineas, para finalmente ordenarlos según la función recibida por parámetro.
Después, explicar un toque parseLineas, y por qué tuvimos que hacer la struct line. Y el diagrama de flujo. FIN!
